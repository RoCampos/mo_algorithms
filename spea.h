...
void Spea2::recombination () {
	...
	if (gen == 2) {
		for (int i=0; i < (all_pop-ARCSIZE); i++) {
			...
			int p1 = rand () % ARCSIZE + (all_pop-ARCSIZE);
			int p2 = rand () % ARCSIZE + (all_pop-ARCSIZE);
			if (p1 == p2) p1 = rand () % ARCSIZE + (all_pop-ARCSIZE);
		
			//this constructor is used to create a empty individual
			GenericIndividual * ind = new GenericIndividual(false);
			ind->index = i;

			
			// AQUI EU DEFINO O CROSSOVER
			//rca::crossover3 (population[p1]->mpp_ind,population[p2]->mpp_ind,&(ind->mpp_ind) );
...
int Spea2::binaryTournament  () {
	...
	int ind1 = rand () % (all_pop - ARCSIZE) + ARCSIZE;
	int ind2 = rand () % (all_pop - ARCSIZE) + ARCSIZE;
	while (ind1 == ind2) ind2 = rand () % (all_pop - ARCSIZE) + ARCSIZE;
